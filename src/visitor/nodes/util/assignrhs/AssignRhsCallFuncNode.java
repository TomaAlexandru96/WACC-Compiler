...
public class AssignRhsCallFuncNode extends AssignRhsNode<WACCParser
        .AssignRhsCallFuncContext> {
    private List<ExprNode> args;
    private String ident;

    // CALL_FUNC IDENT OPEN_PARENTHESES argList? CLOSE_PARENTHESES
    public AssignRhsCallFuncNode(SymbolTable currentST, WACCParser
            .AssignRhsCallFuncContext
    ...
    public List<Instruction> generateInstructions(CodeGenerator codeGenRef,
        ...
        for (int i = args.size() - 1; i >= 0; i--) {
            ...
            instructions.addAll(arg.generateInstructions(codeGenRef,
                    availableRegisters));
            instructions.add(new BaseInstruction(Ins.getStrInstruciton(arg
                    .getType()),
                    availableRegisters.get(0),
                    new StackLocation(true, Register.SP, new Offset(-arg
                            .getType().getSize()))));
            totalSpaceSize += arg.getType().getSize();
            currentST.addExtraSpace(arg.getType().getSize());
        }
        instructions.add(new BaseInstruction(Ins.BL, new LabelOp("f_" +
                ident)));
        // restore space
        instructions.add(new BaseInstruction(Ins.ADD, Register.SP, Register
                .SP, new Offset(totalSpaceSize)));
        currentST.resetExtraSpace();
        instructions.add(new BaseInstruction(Ins.MOV, availableRegisters.get
                (0), Register.R0));
