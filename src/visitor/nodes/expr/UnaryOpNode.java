...
public class UnaryOpNode extends ExprNode<WACCParser.ExprContext> {
    ...
    public List<Instruction> generateInstructions(CodeGenerator codeGenRef,
        ...
        List<Instruction> instructions = new LinkedList<Instruction>() {{
            addAll(argument.generateInstructions(codeGenRef,
                    availableRegisters));
        }};
        ...
        switch (operator) {
            case "-":
                if (!(argument instanceof LiteralNode)) {
                    ...
                    instructions.add(new BaseInstruction(Ins.RSBS, reg, reg,
                            new Offset(0)));
                    instructions.add(new BaseInstruction(Ins.BLVS, new
                            LabelOp(ThrowOverflowError.FUNC_NAME)));
                }
                break;
            case "!":
                instructions.add(new BaseInstruction(Ins.EOR, reg, reg, new
                        Offset(1)));
                break;
            case "len":
